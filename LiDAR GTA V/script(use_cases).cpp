...
void introduceError(Vector3* xyzError, Vector3* xyzErrorDist, double x, double y, double z, double error, int errorDist)
{
	...
	//if (errorDist = 0)
		r = r + 2 * ((rand() % 2) * 2 - 1) * 0.1;
	//else
		rDist = r + 2 * ((rand() % 2) * 2 - 1) * 0.05 * r;
	...
	xyzErrorDist->x = rcos_theta * std::cos(az);
	xyzErrorDist->y = rcos_theta * std::sin(az);
	xyzErrorDist->z = r * std::sin(el);
}
...
void lidar(double horiFovMin, double horiFovMax, double vertFovMin, double vertFovMax, double horiStep, double vertStep, int range, std::string filePath, double error, int errorDist, std::ofstream& log)
{
	...
	log << " Done.\n";
	std::ofstream fileOutput, fileOutputPoints, fileOutputError, fileOutputErrorDist, fileOutputErrorPoints, fileOutputErrorDistPoints;
	fileOutput.open(filePath + ".ply");
	fileOutputPoints.open(filePath + "_points.txt");
	//fileOutputError.open(filePath + "_errorDist.ply");
	//fileOutputErrorDist.open(filePath + "_error.ply");
	fileOutputErrorPoints.open(filePath + "_error.txt");
	fileOutputErrorDistPoints.open(filePath + "_errorDist.txt");
	fileOutput << "ply\nformat ascii 1.0\nelement vertex " + std::to_string((int)vertexCount) + "\nproperty float x\nproperty float y\nproperty float z\nend_header\n";
	//fileOutputError << "ply\nformat ascii 1.0\nelement vertex " + std::to_string((int)vertexCount) + "\nproperty float x\nproperty float y\nproperty float z\nend_header\n";
	//fileOutputErrorDist << "ply\nformat ascii 1.0\nelement vertex " + std::to_string((int)vertexCount) + "\nproperty float x\nproperty float y\nproperty float z\nend_header\n";
	...
	fileOutputErrorDist << std::to_string(centerDot.x) + " " + std::to_string(centerDot.y) + " " + std::to_string(centerDot.z) + " 0 0 0\n";
	...
	std::string vertexData = "", vertexDataPoints = "", vertexError = "", vertexErrorDist = "", vertexErrorPoints = "";
	...
	for (int i = 0; i < 3; i++) {
		...
		for (int j = 0; j < vertexCount; j++)
		{
			...
			Vector3 xyzError, xyzErrorDist;
			introduceError(&xyzError, &xyzErrorDist, voxel.x, voxel.y, voxel.z, error, errorDist);
			...
			if (err_x2d > -1 || err_y2d > -1) {
				...
				//vertexErrorDist += std::to_string(xyzErrorDist.x - centerDot.x) + " " + std::to_string(xyzErrorDist.y - centerDot.y) + " " + std::to_string(xyzErrorDist.z - centerDot.z) + "\n";
	...
	//fileOutputErrorDist << vertexErrorDist;
	fileOutputErrorPoints << vertexErrorPoints;
	fileOutputErrorDistPoints << vertexErrorPoints;
	log << "Done.\n";
	...
	//fileOutputErrorDist.close();
	fileOutputErrorPoints.close();
	fileOutputErrorDistPoints.close();
	log.close();
